...
module.exports = function (outputPath) {
	...
	const prepareParameters = metadata => {
		...
		let ar = getClosestMatchingAspectRatio(flip([video.width, video.height]));
		// 3. find the closest matching resolution for closest matching aspect ratio -
		let res = getClosestMatchingResolution(ar.join(':'), flip([video.width, video.height]));
		// 4. find the ffmpeg parameters for the closest matching resolution
		let options = parameters[res.join('x')];
		...
		if (!options) {
			...
			options = [parameters[resolutionsPerAspectRatio[ar.join(':')][0].join('x')]];
